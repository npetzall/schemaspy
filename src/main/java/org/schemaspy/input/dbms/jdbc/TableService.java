...
import org.schemaspy.input.dbms.exceptions.ColumnInitializationFailure;
import org.schemaspy.input.dbms.exceptions.InconsistencyException;
...
public class TableService {
    ...
    public TableService(DbmsConfiguration dbmsConfiguration, SqlService sqlService) {
    ...
    private TableColumn initColumn(Table table, ResultSet rs) throws SQLException {
        ...
        Pattern excludeIndirectColumns = dbmsConfiguration.getIndirectColumnExclusions();
        Pattern excludeColumns = dbmsConfiguration.getColumnExclusions();
    ...
    protected void addForeignKey(Database db, Table table, String fkName, String fkColName,
        ...
        Pattern include = dbmsConfiguration.getTableInclusions();
        Pattern exclude = dbmsConfiguration.getTableExclusions();
        ...
        if (childColumn != null) {
            ...
            String catalog = db.getCatalog().getName();
            String schema = db.getSchema().getName();
            String baseContainer = schema != null ? schema : catalog != null ? catalog : db.getName();
     ...
     * @throws SQLException
     */
    protected long fetchNumRows(String selectRowCountSql, Database db, Table table) {
        if (table.isView() || table.isRemote())
            return -1;

        SQLException originalFailure = null;

        if (selectRowCountSql != null) {

            try (PreparedStatement stmt = sqlService.prepareStatement(selectRowCountSql, db, table.getName());
                 ResultSet rs = stmt.executeQuery()) {
    ...
    private void initIndexes(Database db, Table table) throws SQLException {
        ...
        if (initIndexesFromSql(db, table))
            return;
     ...
     */
    private boolean initIndexesFromSql(Database db, Table table) {
        String selectIndexesSql = dbmsSql.getSelectIndexesSql();
