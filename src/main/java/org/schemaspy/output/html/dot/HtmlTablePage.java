...
public class HtmlTablePage extends HtmlFormatter {
    ...
    public HtmlTablePage(HtmlConfig config, DotFormatter dotFormatter) {
    ...
    private boolean generateDots(Table table, File diagramDir, WriteStats stats, File outputDir) throws IOException {
        ...
        String extension = dot == null ? config.imageFormat() : dot.getFormat();
        ...
        if (table.getMaxChildren() + table.getMaxParents() > 0) {
            ...
            LineWriter dotOut = new LineWriter(oneDegreeDotFile, config.charset());
            WriteStats oneStats = new WriteStats(stats);
            dotFormatter.writeRealRelationships(table, false, oneStats, dotOut, outputDir);
            dotOut.close();

            dotOut = new LineWriter(twoDegreesDotFile, config.charset());
            WriteStats twoStats = new WriteStats(stats);
            impliedConstraints = dotFormatter.writeRealRelationships(table, true, twoStats, dotOut, outputDir);
            dotOut.close();
            ...
            if (!impliedConstraints.isEmpty()) {
                dotOut = new LineWriter(oneImpliedDotFile, config.charset());
                dotFormatter.writeAllRelationships(table, false, stats, dotOut, outputDir);
                dotOut.close();

                dotOut = new LineWriter(twoImpliedDotFile, config.charset());
                dotFormatter.writeAllRelationships(table, true, stats, dotOut, outputDir);
                dotOut.close();
                return true;
            }
        }
