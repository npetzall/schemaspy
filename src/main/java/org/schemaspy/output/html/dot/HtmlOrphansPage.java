...
public class HtmlOrphansPage extends HtmlDiagramFormatter {
    ...
    public HtmlOrphansPage(HtmlConfig config, DotFormatter dotFormatter) {
    ...
    public boolean write(Database db, List<Table> orphanTables, File diagramDir, File outputDir) throws IOException {
        ...
        try {
            ...
            for (Table table : orphanTables) {
                ...
                LineWriter dotOut = new LineWriter(dotFile, config.charset());
                try {
                    dotFormatter.writeOrphan(table, dotOut, outputDir);
                } catch (IOException e) {
