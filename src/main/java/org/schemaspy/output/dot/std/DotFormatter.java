...
import static org.schemaspy.output.dot.std.DotNodeConfig.dotNodeConfig;
...
public class DotFormatter {
    ...
    private final DotConnectorFinder finder;

    public DotFormatter(DotConfig config) {
        this.config = Objects.requireNonNull(config);
        finder = new DotConnectorFinder(config);
    }
    ...
    private Set<ForeignKeyConstraint> writeRelationships(Table table, boolean twoDegreesOfSeparation, WriteStats stats, boolean includeImplied, LineWriter dot, File outputDir) throws IOException {
        ...
        for (Table relatedTable : relatedTables) {
            ...
            nodes.put(relatedTable, new DotNode(relatedTable, "", outputDir, dotNodeConfig(config).showColumns(true)));
        ...
        if (twoDegreesOfSeparation) {
            for (Table relatedTable : relatedTables) {
                ...
                for (Table cousin : cousins) {
                    ...
                    nodes.put(cousin, new DotNode(cousin, "", outputDir, dotNodeConfig(config)));
                }
        ...
        nodes.put(table, new DotNode(table, "", outputDir, dotNodeConfig(config)
                .showColumns(true)
                .showColumnDetails(true)
                .showTrivialColumns(true)));
    ...
    private void writeHeader(String diagramName, boolean showLabel, LineWriter dot) throws IOException {
        dot.writeln("// SchemaSpy rev " + new Revision() + " on " + System.getProperty("os.name") + " " + System.getProperty("os.version"));
        dot.writeln("digraph \"" + diagramName + "\" {");
        dot.writeln("  graph [");
        boolean rankdirbug = config.rankDirBug();
        if (!rankdirbug)
            dot.writeln("    rankdir=\"RL\"");
        dot.writeln("    bgcolor=\"" + config.bodyBackgroundColor() + "\"");
        if (showLabel) {
            if (rankdirbug)
        ...
        dot.writeln("    fontname=\"" + config.font() + "\"");
        dot.writeln("    fontsize=\"" + config.fontSize() + "\"");
        //dot.writeln("    dpi=80");
    ...
    private boolean writeRelationships(Database db, Collection<Table> tables, boolean compact, boolean showColumns, boolean includeImplied, WriteStats stats, LineWriter dot, File outputDir) throws IOException {
        DotNodeConfig nodeConfig = dotNodeConfig(config)
                .showColumns(showColumns)
                .showTrivialColumns(!compact)
    ...
    public void writeOrphan(Table table, LineWriter dot, File outputDir) throws IOException {
        ...
        dot.writeln(new DotNode(table, "tables/", outputDir, dotNodeConfig(config)
                .showColumns(true)
                .showTrivialColumns(true)).toString());
        dot.writeln("}");
    }
}
